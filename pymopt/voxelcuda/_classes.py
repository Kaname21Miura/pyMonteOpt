...
class BaseVoxelMonteCarlo(MonteCalro,metaclass = ABCMeta):
    ...
    def start(self,rand_seed=1234):
        ...
        func(
            ...
            cuda.InOut(self.v),
            cuda.InOut(self.w),
            cuda.In(self.model.ma.astype(np.float32)),
            cuda.In(self.model.ms.astype(np.float32)),
            cuda.In(self.model.n.astype(np.float32)),
            cuda.In(self.model.g.astype(np.float32)),
            cuda.In(self.model.voxel_model.astype(np.int8)),np.float32(self.model.voxel_space),
            M,L,np.int32(self.nPh),np.int8(self.model.end_point),np.int32(rand_seed),
            grid=(int((self.nPh+self.threadnum-1)/self.threadnum),1), block=(self.threadnum,1,1))
    ...
    def set_monte_params(self,*,nPh,model, dtype='float32',
