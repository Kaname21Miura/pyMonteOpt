...
注意
このファイルは正常に動作しているように見えます。
そして、無限遠媒質での計算結果は、L.Wang 1992の結果と一致します。
しかしながら、媒質が小さい場合、結果に矛盾が生じることがわかっています。
ここで、媒質の大きさはµtの大きさに依存し、µtが極端に小さい場合は大きなサイズの媒質でも
計算結果が保証できない場合があります。

このファイルを残すの理由は、いくつかあります。
１つ目は、矛盾した結果が算出される原因がわかっていません。
本ファイルに含まれるモデルは、一般的には１光子づつ計算させるモンテカルロ法（モデル１）を、
計算の高速化のため、全光子を並行して散乱挙動を１ステップづつ計算させています（モデル２）。
このモデルは、１づつ計算させた場合と比較して、次の点で結果が異なります。
・モデル２で空気と散乱媒質に屈折率の差がある場合、拡散光強度分布（Rd,Tdと半径方向rの比較）がモデル１と異なります。
　また、このとき境界屈折後に単位ベクトルに変換した場合、透過角と光強度の関係に奇形が生じます。
・モデル２で空気と散乱媒質に屈折率の差がない場合、側方方向に透過する光強度分布（L）がモデル１と異なります。
　ここで、µtが大きい場合、モデル１が大きく、µtが小さい場合はモデル２が大きくなります。
　この差は、１光子づつ計算させるか、並行に１ステップづつ計算させるか以外が原因ではありません。これは確認されました。
　この結果は、奇妙でそして興味深い結果です。
これらの結果の差異がなぜ生じるのか、原因は未だ定かではありません。従って、ここにこのファイルを残そうと考えました。

2つ目は、このファイルには、試行錯誤が残っています。例えば、モデル１には、まだフルエンスの計算を導入していません。
また、Dicomeデータを用いた計算らLEDでの計算も可能です。
これらの機能を移植する場合、このファイルで定義された複数の関数は、そのまま利用可能です。
従って、このファイルは、将来の開発に対して何らかの助けになると思われます。
...
class BaseVoxelMonteCarlo(MonteCalro,metaclass = ABCMeta):
    ...
    def _border_act(self,v,p,add,s,db,mt):
        ...
        if list(pb_index) != []:
            ...
            ai = np.arccos(ai[ai!=0]).astype(np.float32)#Aから算出される0を排除して1次元配列にする\
            ...
            at = np.arcsin((ni[pb_index]/nt[pb_index])*np.sin(ai)).astype(np.float32)
            Ra = (0.5*(np.add((np.sin(ai-at)/np.sin(ai+at))**2,
            (np.tan(ai-at)/np.tan(ai+at))**2))).astype(np.float32)
